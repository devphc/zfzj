...
Page({
  ...
  onLoad: function (e) {
    ...
    wx.request({
      url: app.globalData.domains +'/api/goods/detail',
      data: {
        id: e.id
      },
      success: function(res) {
        var selectSizeTemp = "";
        if (res.data.goods.properties) {
        ...
        that.data.goodsDetail = res.data.goods;
        that.setData({
          goodsDetail:res.data.goods,
        });
        WxParse.wxParse('article', 'html', res.data.goods.describe, that, 5);
      }
    })
  ...
  addShopCar:function(){
    ...
    shopCarMap.pic=this.data.goodsDetail.goodsPicList[0].pic;
  ...
  onShareAppMessage: function () {
    return {
      ...
      path: '/pages/goods-detailss/index?id=' + this.data.goodsDetail.basicInfo.id,
      success: function (res) {
        // 转发成功
      },
      fail: function (res) {
